...
public class NonTeachingStaffController {
	...
	public String nonteachlist(@PathVariable("id") Long id, ModelMap nonteachModel) {
		...
		for (NonTeachingStaff nt : ntel) {
			for (Employee e : el) {
				if (nt.getEmpId().equals(e.getEmpId())) {
					reqnonteach.add(e);
					break;
				}
			}
	...
	public String deleteNonTeach(@PathVariable("id") Long id) {
